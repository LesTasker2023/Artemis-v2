...
ARTEMIS v2 uses `electron-updater` with GitHub Releases for automatic updates.
...
1. **Bump version** in `package.json`:
   ...
   ```json
   {
     "version": "2.0.1" // Increment following semver
   }
   ```
...
2. **Build release**:
   ...
   Outputs to `dist-build/`: `.exe`, `.dmg`, `.AppImage`, etc.

3. **Create GitHub Release**:

   - Go to https://github.com/LesTasker2023/ARTEMIS/releases
   - Tag: `v2.0.1` (must match package.json with 'v' prefix)
   - Upload ALL files from `dist-build/` including `latest*.yml` files
   - Write release notes
   - Publish

4. **Users auto-update**: Next app launch checks GitHub, downloads update, prompts to restart
...
2. Bump version, rebuild
3. Create GitHub release (can be draft)
4. Open installed app - should detect update

**Auto-Update Behavior:**

- Checks GitHub every hour + on startup
- Downloads in background
- Shows "Update Ready" dialog
- One-click restart to install
- Disabled in dev mode (`npm run dev:electron`)
...
- [ ] Publish release (users auto-update within 1 hour)

**See `AUTO_UPDATE_GUIDE.md` for detailed documentation.**
